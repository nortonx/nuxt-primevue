<template>
  <div class="formgrid grid">
    <div class="field col-12 md:col-6">
      <label for="firstName" class="block text-900 font-medium mb-1"
        >First Name</label
      >
      <InputText
        id="firstName"
      ...
      />
      <span v-show="$v.formData.firstName.$error" class="error-message">{{
        $v.formData.firstName.required.$message
      }}</span>
    </div>
    <div class="field col-12 md:col-6">
      <label for="lastName" class="block text-900 font-medium mb-1"
        >Last Name</label
      >
      <InputText
        id="lastName"
      ...
      />
      <span v-show="$v.formData.lastName.$error" class="error-message">{{
        $v.formData.lastName.required.$message
      }}</span>
    </div>
    <div class="field col-12 md:col-6">
      ...
      />
      <span v-show="$v.formData.email.$error" class="error-message">{{
    ...
    <div class="field col-12 md:col-6">
      ...
      />
      <span v-show="$v.formData.phone.$error" class="error-message">{{
        $v.formData.phone.required.$message
      }}</span>
    </div>
    <div class="field col-12 md:col-6">
      <label for="password" class="block text-900 font-medium mb-1"
        >Password</label
      >
      <InputText
        id="password"
      ...
      />
      <span v-show="$v.formData.password.$error" class="error-message">{{
        $v.formData.password.required.$message
      }}</span>
    </div>
    <div class="field col-12 md:col-6">
      <label class="block text-900 font-medium mb-1" for="country"
        >Country</label
      >
      <Dropdown
        id="country"
        ...
        class="w-full mb-2"
      />
      <span v-show="$v.formData.country.$error" class="error-message">{{
        $v.formData.country.required.$message
      }}</span>
    </div>
    <div class="errors">
      <code>
        <p v-for="error in $v.$errors" :key="error.$uid">
          {{ error.$property }} : {{ error.$params }}
        </p>
      </code>
    ...
    <div v-if="formStore.records" class="card">
      <ul class="user-list list-none p-0 flex flex-wrap">
        <li
          ...
          :key="record.phone"
          class="user-card m-3 w6"
        >
          <Card>
            <template #title>{{
              `${index + 1} - ${record.firstName} ${record.lastName}`
            }}</template>
            <template #content>
              <div class="row">
...
const rules = {
  formData: {
    ...
    email: {
      required: helpers.withMessage("E-mail with invalid format", required),
      email,
    },
    password: {
